...
def test_simulate_endpoint_full_pipeline(mock_post, mock_insert, mock_get_news):
    ...
    for run in data["runs"]:
        ...
        assert (
            run["fuse_evaluations"][0]["fuse_scores_vs_ground_truth"]["Total_Deviation"]
            == 5.0
        )
    ...
    assert "fuse_evaluations" in saved_result_json["runs"][0], (
        "存入数据库的 Run 丢失了 FUSE 分数！"
    )
...
def test_history_list_endpoint(mock_list):
    ...
    mock_list.return_value = [{"run_id": 1, "created_at": "2026-04-18", "news_id": 1}]
