...
# --- 修改代码开始 ---
# 增加了一个 `text: str` 参数，这样它就不会只用写死的全局变量了
def seed_ground_truth(network, registry: PostRegistry, text: str) -> Post:
    """
...
def run_simulation(
    ...
    seed: int | None,
    out_dir: str,
    ground_truth_text: str = None  # <--- 新增：允许从 API 传文本进来
) -> dict:                         # <--- 修改：声明这个函数会返回一个字典给 main.py
    if seed is not None:
        random.seed(seed)

    # 判断一下，如果前端传了文本就用前端的，如果没传就用上面默认的常量
    active_truth = ground_truth_text if ground_truth_text else GROUND_TRUTH
...
# --- 修改代码开始 ---
    # 把选定的文本传给 seed_ground_truth 进行播种
    seed_post = seed_ground_truth(network, registry, active_truth)
    ...
    run_log = {
        ...
        "ground_truth": {
            "text":    active_truth,  # <--- 修改：日志里记录的也是动态文本
...
# --- 修改代码结束 ---
    ...
    for step in range(1, n_steps + 1):
        ...
        for agent in shuffled:
            ...
            for post in feed:
                # --- 修改代码开始 ---
                # 让 agent 根据你传进来的文本做出反应
                action_result = agent_process_post(agent, post, active_truth)
                # --- 修改代码结束 ---
    ...
    # --- 修改代码开始 ---
    # 最关键的一步：把生成好的大字典返回给 main.py，供前端使用
    return run_log
    # --- 修改代码结束 ---
